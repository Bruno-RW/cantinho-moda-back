...
const Navbar = () => {
    ...
    return (
        <nav className={twMerge("w-72 h-screen overflow-hidden transition-all bg-blue-500 dark:bg-blue-500/80", isActive && "w-[60px] active")}>
            ...
            <ul className="w-full">
                {navbarInfo.map((item, key) => (
                    <li className={twMerge("navbar-li group", isActive && "hover:translate-x-0 hover:pl-2 hover:ml-2")} key={key}>
                        <Link className="flex items-center relative text-gray-50 w-full gap-x-3.5 py-3.5 no-underline group-hover:text-blue-500 dark:text-neutral-50 dark:group-hover:text-blue-500/80" href={item.url}>
